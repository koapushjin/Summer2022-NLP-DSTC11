...
def analyze(datapath):
    info = {}
    in_domain = ['GetQuote', 'FileClaim', 'CreateAccount', 'RequestProofOfInsurance', 'UpdateBillingFrequency', 'PayBill', 'UpdatePaymentPreference', 'FindAgent', 'ReportAutomobileAccident', 'CheckPaymentStatus', 'CancelAutomaticBilling', 'EnrollInPlan', 'CancelPlan', 'AddDependent', 'ChangeAddress', 'GetPolicyNumber', 'RemoveDependent', 'ResetPassword', 'CheckAccountBalance', 'ChangePlan', 'ReportBillingIssue', 'ChangeSecurityQuestion']
    with open(datapath,'r') as f:
        l = json.load(f)
        print(len(l))
        for u in l:
            if u['predicted_label'] not in info.keys():
                info[u['predicted_label']] = {'in_domain':{},'out_of_domain':{}}
            if u['reference_label'] in in_domain:
                k = info[u['predicted_label']]['in_domain']
            else:
                k = info[u['predicted_label']]['out_of_domain']
            if u['reference_label'] not in k.keys():
                k[u['reference_label']]={}
                k[u['reference_label']]['number'] = 0

            k[u['reference_label']]['number'] += 1
            k[u['reference_label']]['utterance'] = u['utterance']
    ds = open("analysis_v2.json","w")
    json.dump(info,ds)
    ds.close()

def print_metrics(datapath):
    ref = []
    pred = []
    metrics = [
            NMI(),
    ...
    with open(datapath,'r') as f:
        ...
        print(len(l))
        for u in l:
            ref.append(u['reference_label'])
            pred.append(u['predicted_label'])

    cluster_labels = pred
    reference_labels = ref
    cluster_labels, reference_labels = filter_labels(cluster_labels, reference_labels,ignored_labels = [])
    metrics = {
    ...
    # with open(datapath+'.metrics.json','w') as f:
    #     json.dump(metrics,f)
    return metrics
...
import pickle
def encoding(model_path,utterances):
    encoder = SentenceTransformer(model_path)
    encoded_utt =  encoder.encode(utterances)
    with open('encoded','wb') as f:
        pickle.dump(encoded_utt,f)
    return encoded_utt

def get_all_utterances(file_name):
    utts = []
    labels = []
    ids = []
    with open(file_name,'r') as f:
        ...
        for d in dialogues:
            d = json.loads(d)
            for turn in d['turns']:
                if len(turn['utterance']) > 15:
                    utts.append(turn['utterance'])
                    try:
                        labels.append(turn['intents'][0])
                    except:
                        labels.append('')
                    ids.append(turn['turn_id'])
    with open("../tsdae/data/all_utterances.jsonl","w") as j:
        for i in range(len(utts)):
            d = {"intent":labels[i],"utterance":utts[i],"turn_id":ids[i]}
            j.write(json.dumps(d)+'\n')
    return utts, labels, ids

def get_utterances(file_name):
    utts = []
    labels = []
    ids = []
    with open(file_name,'r') as f:
...
import math
def clustering(algo,utts,labels,n_clusters):
    if algo == 'kmeans':
        clusterer = KMeans(n_clusters = n_clusters, init='k-means++')
        results = clusterer.fit_predict(utts)
        return results



def test(pred_path):
    metric = print_metrics(pred_path)
    with open(pred_path+'.metrics.json','w') as f:
        json.dump(metric,f)
