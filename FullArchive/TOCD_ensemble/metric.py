...
def get_metrics(result_path,domain,num_trials):
    utts,labels,ids = get_utterances('data/test-{}/dialogues.jsonl'.format(domain))
    ...
    for id in ids:
        for l in L:
            if l['turn_id'] == id:
                encoded_utts.append(l['hidden_state'])
                break
    encoded_utts = torch.tensor(encoded_utts)
    ...
    for n_clusters in range(5,50):
        ...
        for i in range(num_trials):
            results = clustering('kmeans',encoded_utts,labels,n_clusters)
            info = []
            for i in range(len(utts)):
                info.append({'predicted_label':str(results[i]),'reference_label':labels[i],'turn_id':ids[i],'utterance':utts[i]})
            metric = print_metrics(info)
            if metric['ACC'] > best_acc:
                best_acc = metric['ACC']
                best_metric = metric
                best_info = info
        metrics['N_CLUSTER={}'.format(n_clusters)] = best_metric
    pred_path = "/".join(result_path.split("/")[:-1]) + "/predictions.json"
    with open(pred_path,'w') as j:
        for pred in best_info:
            j.write(json.dumps(pred)+'\n')

    metric_path = "/".join(result_path.split("/")[:-1]) + "/metrics.json"
    with open(metric_path,'w') as j:
        json.dump(metrics,j)
