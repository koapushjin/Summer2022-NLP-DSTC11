...
class Glue(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        GlueConfig(
            ...
            description=textwrap.dedent(
                """\
            DSTC10 title-body pair validation task"""
            ),  # pylint: disable=line-too-long
            text_features={"sentence1": "", "sentence2": ""},
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.name == "dstc10":
            data_dir = None
            dstc10_files = {
                    "train": "classifier_examples_train.json",
                    "test": "classifier_examples_test.json",
            }
        else:
    ...
    def _generate_examples(self, data_file, split, mrpc_files=None, dstc10_files=None):
        if self.config.name == "dstc10":
            examples = self._generate_example_dstc10_files(dstc10_files=dstc10_files, split=split)
            ...
            with open(data_file, encoding="utf8") as f:
                ...
                for n, row in enumerate(reader):
                    ...
                    for value in example.values():
                        ...
                        yield example["idx"], example
    '''
    def _generate_example_dstc10_files(self, dstc10_files, split):
        if split =="test":
            with open(dstc10_files["test"]) as f:
                dic=json.load(f)
                # this is for iteratiing json data
                for n,elem in enumerate(dic):
                    xlabel=0
                    if elem['label']=='true':
                        xlabel=1
                    yield{
            ...
            with open(dstc10_files["train"]) as f:
                dic=json.load(f)
                for n,elem in enumerate(dic):
                    xlabel=0
                    if elem['label']=='true':
